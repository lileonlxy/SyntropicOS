...
void test_tls_x509_mode_handshake(void)
{
    ...
    /* Test valid DER with chain validation failure */
    SYN_TLS_Config bad_chain_cfg = {.mode = SYN_TLS_AUTH_MODE_X509_SERVER,
                                    .server_name = "mismatch.server.com",
                                    .root_ca = &root_ca,
                                    .client_cert_der = ec_cert_der,
                                    .client_cert_len = sizeof(ec_cert_der)};
    SYN_TLS_Context bad_chain_tls;
    TEST_ASSERT_TRUE(syn_tls_init(&bad_chain_tls, &bad_chain_cfg, &tr, rx_record_buf,
