/**
 * @file syn_usb_host.c
 * @brief Zero-Heap USB 2.0 Host Core Protocol Engine Implementation.
 *
 * Tick-driven state machine handling device attach detection, bus reset,
 * standard USB enumeration (Device/Config descriptor reads, SET_ADDRESS,
 * SET_CONFIGURATION), and host class driver interface matching.
 */
...
/**
 * @brief Build a standard USB control Setup Packet.
 * @param pkt Target Setup Packet pointer.
 * @param bmRequestType Request characteristics bitfield.
 * @param bRequest Specific request code.
 * @param wValue Word-sized request parameter.
 * @param wIndex Word-sized index or offset.
 * @param wLength Transfer byte length.
 */
...
/**
 * @brief Walk configuration descriptor and probe registered class drivers
 *        against each interface descriptor's class/subclass/protocol triple.
...
/**
 * @brief Drive the enumeration sub-state machine. Non-blocking.
 *        Each call performs at most one HAL operation.
 *
 * @param host Pointer to USB Host instance.
 * @return SYN_OK when enumeration complete, SYN_BUSY while in progress,
 *         SYN_ERROR on failure.
 */
