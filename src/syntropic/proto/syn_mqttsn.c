/**
 * @file syn_mqttsn.c
 * @brief MQTT for Sensor Networks (MQTT-SN v1.2) Protocol Client Engine Implementation.
 */
...
/**
 * @brief Encode MQTT-SN packet header into buffer.
 * @param buf Output buffer.
 * @param max_buf Maximum buffer capacity.
 * @param msg_type MQTT-SN message type.
 * @param payload_len Length of payload that follows the header.
 * @return Header length in bytes (2 or 4), or 0 on buffer overflow.
 */
...
/**
 * @brief Decode MQTT-SN packet header from buffer.
 * @param buf Input buffer.
 * @param len Buffer length.
 * @param out_msg_type Output message type.
 * @param out_payload_offset Output byte offset to payload.
 * @param out_payload_len Output payload length.
 * @return true if valid header, false if incomplete or malformed.
 */
...
/**
 * @brief Find or allocate topic entry in client table.
 * @param client Client instance.
 * @param topic_name Topic string.
 * @return Pointer to topic entry, or NULL if table is full.
 */
...
/**
 * @brief Internal helper to publish with configurable topic ID type flag.
 * @param client Client instance pointer.
 * @param topic_id Topic ID or 2-byte short topic name.
 * @param topic_type_flag Topic ID type flag (Normal, Predefined, Short).
 * @param qos QoS level (0, 1, -1).
...
/**
 * @brief Handle incoming decoded MQTT-SN message frame.
 * @param client Client instance pointer.
 * @param msg_type Decoded MQTT-SN message type.
