...
/**
 * @file syn_aes.c
 * @brief Unified AES block cipher & AEAD implementation (128/192/256-bit keys, ECB, CBC, CTR, GCM).
 */
...
/**
 * @brief Multiply byte by 2 in GF(2^8) modulo x^8 + x^4 + x^3 + x + 1.
 * @param a Input byte.
...
/**
 * @brief Increment 128-bit big-endian counter.
 * @param counter 16-byte counter block to increment in place.
 */
...
/**
 * @brief Bitwise GHASH field multiplication in GF(2^128).
 * @param x First 16-byte block.
 * @param y Second 16-byte block.
 * @param out Output 16-byte block product.
 */
...
/**
 * @brief Increment rightmost 32 bits of 128-bit counter.
 * @param block 16-byte block whose last 4 bytes are incremented.
 */
...
/**
 * @brief Write 64-bit value big-endian.
...
/**
 * @brief Process data blocks through GHASH.
 * @param ctx GCM context.
 * @param data Input data buffer.
 * @param len Data length in bytes.
 * @param y In/out 16-byte GHASH state accumulator.
 */
...
/**
 * @brief Compute initial J0 counter block for GCM.
 * @param ctx GCM context.
 * @param nonce Nonce buffer.
 * @param nonce_len Nonce length in bytes.
 * @param j0 Output 16-byte J0 block.
 */
